...
dataset = {}
dataset['train'] = load_data('./data/prepro/train_multiclass.csv', label='q'+str(QID)+'_label')
dataset['dev'] = load_data('./data/prepro/dev_multiclass.csv', label='q'+str(QID)+'_label')
dataset['test'] = load_data('./data/prepro/test_multiclass.csv', label='q'+str(QID)+'_label')
...
disp.plot()
plt.savefig('./results/curriculum_multiclass_'+FORM+'_'+str(QID)+'.png')#save image

report = classification_report(alllabels, allpreds, digits=4)
my_open = open('./results/curriculum_multiclass_'+FORM+'_'+str(QID)+'.txt', 'w')
