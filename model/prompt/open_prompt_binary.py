...
dataset = {}
dataset['train'] = load_data('./data/prepro/train_binary.csv')
dataset['dev'] = load_data('./data/prepro/dev_binary.csv')
dataset['test'] = load_data('./data/prepro/test_binary.csv')

plm, tokenizer, model_config, WrapperClass = load_plm("bert", "princeton-nlp/sup-simcse-bert-base-uncased")
questions = ['', 'Does the tweet contain a verifiable factual claim?','To what extent does the message appear to contain false information?','Will the claim be of interest to the public?',
